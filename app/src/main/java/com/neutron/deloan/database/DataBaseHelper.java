package com.neutron.deloan.database;
...
import com.neutron.deloan.database.entity.ContactEntity;
import com.neutron.deloan.database.entity.SmsEntity;
...
public class DataBaseHelper {
     ...
     */
    public static String[] queryContactByUri(Uri uri) {
        String[] data = null;
        if (uri != null) {
    ...
    public static ArrayList<PContacts> getAllContacts(Context context) {
        ArrayList<PContacts> contacts = new ArrayList<PContacts>();

        Cursor cursor = context.getContentResolver().query(
                ContactsContract.Contacts.CONTENT_URI, null, null, null, null);
        while (cursor.moveToNext()) {
            //新建一个联系人实例
            PContacts temp = new PContacts();
            ...
            temp.setOther_name(name);
            temp.setLast_time(time);
...
//            noteCursor.close();
    ...
    /**
     * 获取短信记录      implementation "com.annimon:stream:1.1.4"
     */
    @SuppressLint("MissingPermission")
     ...
     * @return
     */
    public static List<ApplistEntity> getAppList(Context context) {
        long startTime = System.currentTimeMillis(); //起始时间
        List<ApplistEntity> appLists = new ArrayList<>();
        ApplistEntity appList;
        ...
        for (PackageInfo packageInfo : list) {
            ...
            boolean isSYS = isSystemApp(packageInfo);
            if (!TextUtils.isEmpty(appName) && !TextUtils.isEmpty(packageName)) {
                appList = new ApplistEntity();
                appList.setFirstTime(formatTime(packageInfo.firstInstallTime, "yyyy-MM-dd HH:mm:ss"));
                appList.setLastTime(formatTime(packageInfo.lastUpdateTime, "yyyy-MM-dd HH:mm:ss"));
                appList.setName(appName);
                appList.setPackageName(packageName);
                appList.setVersionCode(packageInfo.versionName);
                appList.setSystemApp(isSYS ? "1" : "2");
                appLists.add(appList);
            }
        }
        long endTime = System.currentTimeMillis(); //结束时间
        Log.i("test", String.format(appLists.size() + "方法使用时间 %d ms", endTime - startTime)); //打印使用时间
        return appLists;
    }
